// app/people/[slug]/page.jsx - NEW PERSON DETAIL PAGE WITH SLUG FORMAT
"use client";
...
// Utility function untuk parse slug menjadi ID
const parsePersonSlug = (slug) => {
  const parts = slug.split('-');
  const lastPart = parts[parts.length - 1];
  
  // Cek jika bagian terakhir adalah ID numerik
  if (!isNaN(parseInt(lastPart))) {
    return parseInt(lastPart);
  }
  
  // Jika tidak ada ID numerik, return null (akan menyebabkan 404)
  return null;
};
...
export default function PersonDetailPage() {
  ...
  // Parse slug untuk mendapatkan person ID
  const personId = parsePersonSlug(slug);

  useEffect(() => {
    if (personId) {
      fetchPersonData();
    } else {
      setError('Invalid person URL');
      setLoading(false);
    }
  }, [personId]);
  ...
  // Utility functions untuk membuat slug movie/TV (untuk link ke detail movie/TV)
      ...
      <div className="min-h-screen bg-slate-900 py-8">
        <div className="container mx-auto px-4 text-center">
          <div className="bg-red-900/50 border border-red-700 text-red-200 px-6 py-8 rounded-xl max-w-2xl mx-auto">
            ...
            <p className="mb-6">{error || 'Invalid person URL format'}</p>
  ...
  const CreditItem = ({ credit, type = 'movie' }) => {
    ...
    // Buat slug untuk link ke detail movie/TV
    const slug = isTV ? createTvSlug(credit) : createMovieSlug(credit);
    
    return (
      <div className="bg-slate-800 rounded-lg p-4 hover:bg-slate-700 transition-all duration-300 group border border-slate-700">
        <Link href={isTV ? `/tv/${slug}` : `/movie/${slug}`}>
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-white hover:text-orange-400 transition-colors line-clamp-2 text-lg leading-tight">
                {title}
              </h3>
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row gap-8 mb-8">
          ...
          <div className="flex-1">
            ...
            {person.biography && (
              <div className="mb-6">
                ...
                <div className="relative">
                  <p className="text-gray-300 leading-relaxed line-clamp-6 transition-all duration-300 bg-slate-800 rounded-lg p-4 border border-slate-700">
                    {person.biography}
                  </p>
            ...
            {/* Popularity */}
            {person.popularity && (
              <div className="flex items-center gap-3 bg-slate-800 px-4 py-3 rounded-lg inline-flex border border-slate-700">
